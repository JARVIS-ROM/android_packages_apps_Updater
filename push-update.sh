...
updates_dir=/data/jarvis_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have jarvis-VERSION-*-TYPE-DATE-* as format"
...
# Assume jarvis-VERSION-*-TYPE-DATE-*.zip
...
adb shell "killall org.jarvis.updater2>/dev/null"
adb shell "sqlite3 /data/data/org.jarvis.updater/databases/updates.db" \
